...
function NoteForm({ onAdded, briefs }: { onAdded: (note: Note) => void; briefs: Brief[] }) {
  const [category, setCategory] = useState<string>('General')
  const [content, setContent] = useState('')
  const [targetBriefId, setTargetBriefId] = useState<string | null>(null)
  ...
  async function submit(e: React.SyntheticEvent) {
    ...
    try {
      const res = await fetch('/api/internal/notes', {
        ...
        body: JSON.stringify({ category, content, briefId: targetBriefId }),
      })
      if (res.ok) {
    ...
    <form onSubmit={submit} className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <p className="text-[10px] text-gray-400">Tag to brief:</p>
          <select
            value={targetBriefId ?? ''}
            onChange={e => setTargetBriefId(e.target.value || null)}
            className="text-xs border border-gray-200 rounded-lg px-2.5 py-1.5 bg-white text-gray-700 outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Future brief</option>
            {briefs.map(b => (
              <option key={b.id} value={b.id}>Week of {b.weekOf}</option>
            ))}
          </select>
        </div>
        <button
      ...
      {tab === 'notes' && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <NoteForm onAdded={note => setNotes(prev => [note, ...prev])} briefs={briefs} />
          </div>
