...
/**
 * Strips prompt-injection patterns before inserting user text into the Claude prompt.
 * Also hard-caps length so a single note can't dominate the context.
 */
...
export async function buildInternalSignals(companyId: string): Promise<string> {
  ...
  if (notes.length > 0) {
    const lines = notes.map(n => `[${n.category}] ${sanitize(n.content)}`)
