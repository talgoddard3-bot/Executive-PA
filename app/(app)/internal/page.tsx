...
export default async function InternalPage() {
  ...
  const [notesResult, docsResult, briefsResult] = await Promise.all([
    supabase
      .from('internal_notes')
    ...
    <div className="p-4 md:p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <InternalPageClient
          ...
          briefs={(briefsResult.data ?? []).map(b => ({
            id: b.id,
            weekOf: new Date(b.week_of).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
          }))}
        />
      </div>
    </div>
